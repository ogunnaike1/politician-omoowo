...
/* ══════════════════════════════════════════
   DATA TYPES
══════════════════════════════════════════ */
export type EventTypeLabel = "Rally" | "Town Hall" | "Forum" | "Summit" | "Consultation";
...
const TYPE_PILL: Record<EventTypeLabel, { bg: string; color: string }> = {
...
══════════════════════════════════════════ */
function EventsHero({ nextEvent }: { nextEvent: NextEventInfo | null }) {
            ...
            >
              ...
              >
                ...
                <p className="text-[10px] tracking-[0.2em] uppercase mb-8" style={{ color: "rgba(246,246,246,0.3)" }}>
                  {nextEvent.dateLabel} &middot; {nextEvent.timeLabel}
                </p>
...
function UpcomingEvents({ events }: { events: CEvent[] }) {
    ...
    <section ref={ref} className="py-24 md:py-40 px-6 md:px-12 lg:px-20" style={{ background: "#FFFFFF" }}>
      <div className="max-w-300 mx-auto">
        ...
        <div className="min-h-50">
          {filtered.length === 0 && (
            <p className="text-sm text-[#888888] py-10">No upcoming events scheduled right now.</p>
          )}
...
function PastEvents({ events }: { events: PastEventItem[] }) {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: "-80px" });
          ...
          {events.map((event, i) => (
            ...
            >
              <div>
                ...
                <div className="w-4 h-0.5 mb-3" style={{ background: pastAccent[i % pastAccent.length] }} />
