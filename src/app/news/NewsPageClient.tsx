...
/* ══════════════════════════════════════════
   DATA
══════════════════════════════════════════ */
export type NewsCategory = "Press Release" | "Campaign Update" | "Community" | "Statement" | "Speech";
...
══════════════════════════════════════════ */
function LeadStory({ lead }: { lead: NewsItem }) {
...
function FeaturedPair({ items }: { items: NewsItem[] }) {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: "-80px" });
...
function NewsIndex({ items }: { items: NewsItem[] }) {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: "-80px" });
  ...
  const subscribe = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setSubError(body.error || "Something went wrong. Please try again.");
        return;
      }
      setSent(true);
    } catch {
                ...
                <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <div className="flex flex-col sm:flex-row gap-0" style={{ border: "1px solid rgba(26,26,26,0.12)" }}>
                    ...
                    <button
                      ...
                      className="px-7 py-4 text-[10px] tracking-[0.22em] uppercase shrink-0 transition-colors duration-200 hover:bg-[#008B4D] focus-visible:outline-none focus-visible:bg-[#008B4D] disabled:opacity-60"
  ...
  if (news.length === 0) {
    return (
      <section className="min-h-screen flex items-center justify-center px-6" style={{ background: "#FFFFFF" }}>
        <p className="text-[#888888] text-sm">No dispatches published yet. Check back soon.</p>
      </section>
    );
  }

  const lead = news[0];
  const featured = news.slice(1, 3);
  const index = news.slice(3);

  return (
    <>
      <NewsHero />
      <LeadStory lead={lead} />
      {featured.length > 0 && <FeaturedPair items={featured} />}
      {index.length > 0 && <NewsIndex items={index} />}
      <NewsletterCTA />
    </>
  );
}
